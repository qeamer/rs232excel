...
| **E1** | Ledning / DuPont | **A**-skrue **2** → **C**-skrue **3** (StarTech RX; prøv C2 hvis null data) |
...
3. **Inn på C = DB25 hunn-terminal (lytte):** **E1** → skrue **C3** (= StarTech pin 3 RX). **E2** → **C7**.  
   Ingen data? Prøv **C2** i stedet for C3.  
...
Test: `python3 read_package.py --bare-fangst --port /dev/ttyUSB0`
